#!/bin/bash
# IPsecGuard AI — FULL end-to-end run: testbed -> capture -> dataset -> eval
...
# ---- two-proposal config used by the downgrade demo & FS proof ----
write_cfg(){  # $1=container $2=left $3=right $4=leftid $5=rightid $6=auto $7=extra
...
if ls testbed/captures/*.pcap >/dev/null 2>&1; then
  echo "${G}captures exist ($(ls testbed/captures/*.pcap | wc -l) files) — skipping ~60-min sweep.${R}"
  echo "  (regenerate: rm testbed/captures/*.pcap then re-run)"
else
  echo "${Y}running sweep --repeat 3 (~60 min, grab a coffee)...${R}"
  cd testbed && python3 sweep.py --repeat 3 && cd ..
fi

sec "4. Build dataset from captures"
...
sub "configuring moon->sun with strong+weak proposals"
...
sub "running the downgrade + scoring"
( cd testbed/attacker && python3 demo_downgrade.py ../../testbed/captures/demo_handshake.pcap ) || \
  echo "${Y}downgrade demo skipped (no valid handshake captured)${R}"

sec "8. LIVE DEMO — Forward-Secrecy Proof"
sub "configuring PFS-on tunnel with fast rekey"
...
if docker exec ipg_moon ipsec statusall 2>/dev/null | grep -q ESTABLISHED; then
  ...
  echo "${Y}tunnel not established — run FS proof manually:${R}"
